...
#/app/bin/cx scan create --project-name "${PROJECT_NAME}" -s . --branch "${BRANCH#refs/heads/}" --scan-info-format json --agent "Bobby Test" --file-filter !.github,!package.json,!yarn.lock | tee -i $output_file
/app/bin/cx scan create --project-name "${PROJECT_NAME}" -s . --branch "${BRANCH#refs/heads/}" --scan-info-format json --agent "Bobby Test" "${arr[@]}" | tee -i $output_file
